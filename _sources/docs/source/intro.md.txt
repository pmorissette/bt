...
First, we will download some data. By default, {py:func}`bt.get (alias for ffn.get) <ffn.data.get>` downloads the Adjusted Close from Yahoo! Finance. We will download some data starting on January 1, 2010 for the purposes of this demo.
...
Once we have our data, we will create our strategy. The {py:class}`Strategy <bt.core.Strategy>` object contains the strategy logic by combining various {py:class}`Algos <bt.core.Algo>`.
...
Finally, we will create a {py:class}`Backtest <bt.backtest.Backtest>`, which is the logical combination of a strategy with a data set.
...
Now we can analyze the results of our backtest. The {py:class}`Result <bt.backtest.Result>` object is a thin wrapper around [ffn.GroupStats](http://pmorissette.github.io/ffn/ffn.html#ffn.core.GroupStats) that adds some helper methods.
