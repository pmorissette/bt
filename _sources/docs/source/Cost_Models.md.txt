...
By default {py:class}`bt.Backtest <bt.backtest.Backtest>` charges no transaction
...
{py:class}`CostModel <bt.core.CostModel>` instance in place of the callable. Two
implementations ship with bt:

- {py:class}`SqrtCostModel <bt.core.SqrtCostModel>` — the empirical square-root
  law of impact (Tóth et al. 2011).
- {py:class}`AlmgrenChrissCostModel <bt.core.AlmgrenChrissCostModel>` — the
