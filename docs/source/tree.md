...
In addition to the concept of {py:class}`Algos <bt.core.Algo>` and {py:class}`AlgoStacks <bt.core.AlgoStack>`, a tree structure lies
...
This diagram represents the strategy we tested in the {doc}`overview example <intro>`. A simple {py:class}`strategy <bt.core.Strategy>`
with two children that happen to be {py:class}`securities <bt.core.SecurityBase>`. However, children nodes don't have to be
...
The base class for nodes in the tree is {py:class}`Node <bt.core.Node>`, and these
can be either of type {py:class}`StrategyBase <bt.core.StrategyBase>` or
{py:class}`SecurityBase <bt.core.SecurityBase>`.
...
For more information, see the APIs for {py:class}`Node <bt.core.Node>`,
{py:class}`SecurityBase <bt.core.SecurityBase>` and {py:class}`StrategyBase <bt.core.StrategyBase>`.

There are two main sub-types of {py:class}`StrategyBase <bt.core.StrategyBase>`:
: - {py:class}`Strategy <bt.core.Strategy>`: Market-value weighted strategy based on
{py:class}`Algos <bt.core.Algo>`.

- {py:class}`FixedIncomeStrategy <bt.core.FixedIncomeStrategy>`: Notional weighted
  strategy based on {py:class}`Algos <bt.core.Algo>`.

There are also two main sub-types of {py:class}`SecurityBase <bt.core.SecurityBase>`:
: - {py:class}`Security <bt.core.Security>`: Standard security. If used within a
{py:class}`FixedIncomeStrategy <bt.core.FixedIncomeStrategy>`, its notional weight
is equal to market value. i.e. common stock.

- {py:class}`CouponPayingSecurity <bt.core.CouponPayingSecurity>`: A security that
...
When using {py:class}`FixedIncomeStrategy <bt.core.FixedIncomeStrategy>`,
...
{py:class}`Strategy <bt.core.Strategy>`.

> - {py:class}`FixedIncomeSecurity <bt.core.FixedIncomeSecurity>`: A
>   {py:class}`Security <bt.core.Security>` for which position (rather than market value)
>   will be used as the notional weight, i.e. a zero-coupon bond.
> - {py:class}`HedgeSecurity <bt.core.HedgeSecurity>`: A {py:class}`Security <bt.core.Security>`
>   for which the notional weight is zero, i.e. an ETF hedge in a bond strategy.
> - {py:class}`CouponPayingHedgeSecurity <bt.core.CouponPayingHedgeSecurity>`:
>   A {py:class}`CouponPayingSecurity <bt.core.CouponPayingSecurity>`
...
which will be automatically converted to instances of {py:class}`Security <bt.core.Security>`
