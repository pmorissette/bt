...
`bt` is also available on [Conda Forge](https://conda-forge.org/), and installable via:

```bash
$ conda install bt --channel conda-forge
```

`bt` is compatible with Python >=3.9.

## Recommended Setup

We believe the best environment to develop with bt is the [IPython Notebook](http://ipython.org/notebook.html). From their homepage, the IPython Notebook
is:
